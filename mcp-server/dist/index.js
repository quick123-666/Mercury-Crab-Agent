...
// Memory Tools
// ============================================================================
server.tool("memory_read", "Read a memory file. Supports daily logs (memory/YYYY-MM-DD.md), MEMORY.md, and user memory.", {
    ...
    try {
        ...
        if (!fs.existsSync(filePath)) {
            return {
                content: [{ type: "text", text: `Memory file not found: ${file}\nSearch in ${MEMORY_DIR}` }],
            };
        }
        ...
        return { content: [{ type: "text", text: `Error reading memory: ${err.message}` }] };
    }
});
server.tool("memory_write", "Write to a memory file. Appends to existing files or creates new ones. Use for daily logs, long-term memory, and user memory.", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `Memory written: ${file} (${mode} mode)` }] };
    }
    catch (err) {
        return { content: [{ type: "text", text: `Error writing memory: ${err.message}` }] };
    }
});
server.tool("memory_search", "Search across all memory files for a keyword or phrase. Returns matching files with context.", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `Error searching memory: ${err.message}` }] };
    }
});
// ============================================================================
// Wiki (Knowledge Base) Tools
// ============================================================================
server.tool("wiki_search", "Search the local knowledge base (wiki directory) for information. Returns matching wiki pages.", {
    ...
    try {
        ...
        searchDir(WIKI_DIR);
        if (results.length === 0) {
            return { content: [{ type: "text", text: `No wiki pages found for "${query}". Try ingesting files from raw/ directory.` }] };
        }
        return { content: [{ type: "text", text: `Found ${results.length} wiki results for "${query}":\n\n${results.join("\n\n")}` }] };
    }
    catch (err) {
        return { content: [{ type: "text", text: `Error searching wiki: ${err.message}` }] };
    }
});
server.tool("wiki_read", "Read a specific wiki page by path.", {
    ...
    try {
        ...
        if (!fs.existsSync(fullPath)) {
            return { content: [{ type: "text", text: `Wiki page not found: ${wikiPath}` }] };
        }
        const content = fs.readFileSync(fullPath, "utf-8");
        return { content: [{ type: "text", text: content }] };
    }
    catch (err) {
        return { content: [{ type: "text", text: `Error reading wiki: ${err.message}` }] };
    }
});
server.tool("wiki_write", "Write or create a wiki page in the knowledge base.", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `Wiki page written: ${wikiPath}` }] };
    }
    catch (err) {
        return { content: [{ type: "text", text: `Error writing wiki: ${err.message}` }] };
    }
});
server.tool("wiki_ingest", "Ingest a file or directory into the wiki knowledge base. Copies files from raw/ or specified path into wiki/.", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `Ingested ${count} files from ${sourcePath} to ${destPath || "projects/"}` }] };
    }
    catch (err) {
        return { content: [{ type: "text", text: `Error ingesting: ${err.message}` }] };
    }
});
server.tool("wiki_status", "Get the current status of the knowledge base: page count, directory structure, recent pages.", {}, async () => {
    try {
        ensureDir(WIKI_DIR);
        let pageCount = 0;
        const pages = [];
        function countDir(dir) {
            ...
            for (const entry of entries) {
                ...
                if (stat.isDirectory()) {
                    ...
                    pageCount++;
                    if (pages.length < 10)
                        pages.push(fullPath.replace(WIKI_DIR + path.sep, ""));
                }
            }
        }
        countDir(WIKI_DIR);
...
${pages.map((p) => `  - ${p}`).join("\n")}`;
        ...
        return { content: [{ type: "text", text: `Error getting wiki status: ${err.message}` }] };
    }
});
// ============================================================================
// Heartbeat Tool
// ============================================================================
server.tool("heartbeat_check", "Run a heartbeat check. Checks what tasks need to be executed based on last execution times. Returns recommended next action.", {}, async () => {
    try {
        ...
        return { content: [{ type: "text", text: `Error running heartbeat: ${err.message}` }] };
    }
});
// ============================================================================
// Self-Improving Tools
// ============================================================================
server.tool("correction_log", "Log a correction to the self-improving corrections.md file. Use when the agent makes a mistake or is corrected.", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `Correction logged. Total corrections: ${count}. If similar corrections reach 3+, consider promoting to a rule in memory.md.` }] };
    }
    catch (err) {
        return { content: [{ type: "text", text: `Error logging correction: ${err.message}` }] };
    }
});
server.tool("pattern_scan", "Scan corrections for repeated patterns. If the same type of error appears 3+ times, suggests promoting to a validated rule.", {}, async () => {
    try {
        const correctionsPath = path.join(SELF_IMPROVING_DIR, "corrections.md");
        if (!fs.existsSync(correctionsPath)) {
            return { content: [{ type: "text", text: "No corrections file found. Nothing to scan." }] };
        }
        const content = fs.readFileSync(correctionsPath, "utf-8");
        ...
        const issues = entries
            ...
            .filter(Boolean);
        if (issues.length === 0) {
            return { content: [{ type: "text", text: "No correction entries found." }] };
        }
        ...
        return { content: [{ type: "text", text: `Error scanning patterns: ${err.message}` }] };
    }
});
server.tool("session_title_gen", "Generate titles for sessions that lack them. Scans all sessions, extracts titles from summary content or generates from first meaningful text.", {
    recent: z.number().optional().default(0).describe("Only process the N most recently modified sessions. 0 = process all."),
}, async ({ recent }) => {
    try {
        if (!fs.existsSync(SESSIONS_DIR)) {
            return { content: [{ type: "text", text: "Sessions directory not found." }] };
        }
        ...
        return { content: [{ type: "text", text: `Error generating session titles: ${err.message}` }] };
    }
});
server.tool("session_summary", "Process sessions and extract summaries. Scans for recent sessions, extracts key topics and decisions, writes condensed summaries to memory directory.", {
    ...
    try {
        ...
        for (const session of toProcess) {
            ...
            for (const summaryFile of summaryFiles) {
                ...
                if (taskMatch)
                    extracted += `## Task: ${taskMatch[1].trim()}\n\n`;
                if (resultMatch)
                    extracted += `## Results: ${resultMatch[1].trim()}\n\n`;
                if (conclusionMatch)
                    extracted += `## Conclusion: ${conclusionMatch[1].trim()}\n`;
        ...
        return { content: [{ type: "text", text: `Error processing session summaries: ${err.message}` }] };
    }
});
server.tool("memory_list", "List all memory files with their sizes and last modified dates.", {}, async () => {
    try {
        ensureDir(MEMORY_DIR);
        const files = [];
        ...
        return { content: [{ type: "text", text: `Memory files (${files.length}):\n\n${files.join("\n")}` }] };
    }
    catch (err) {
        return { content: [{ type: "text", text: `Error listing memory: ${err.message}` }] };
    }
});
// ============================================================================
// Start Server
